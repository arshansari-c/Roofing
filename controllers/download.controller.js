  ...
  path.segments.forEach((segment, i) => {
    ...
    if (foldType !== 'None') {
      ...
      if (length) {
        ...
        let normalX = -unitY; // Default to outside
        let normalY = unitX;
        if (borderOffsetDirection === 'inside') {
          normalX = unitY; // Corrected for inside
          normalY = -unitX;
        }
  ...
  try {
    ...
    if (firstPagePaths > 0) {
      ...
      for (let i = 0; i < firstPagePaths; i++) {
        ...
        const yPos = startY + row * (imgSize + gap + 180);
