...
const calculateTotalFolds = (path) => {
  let totalFolds = (path.angles || []).length; // Base folds from angles
...
const generateSvgString = (path, bounds, scale, showBorder, borderOffsetDirection) => {
  ...
  // Normalization with higher threshold for better quality
  const width = bounds.maxX - bounds.minX;
  const height = bounds.maxY - bounds.minY;
  const maxDim = Math.max(width, height);
  let normalizeScale = 1;
  if (maxDim > 20000) { // Increased threshold to handle larger diagrams
    normalizeScale = 20000 / maxDim;
  }
    ...
    for (let pageIndex = 0; pageIndex < imagePagesNeeded; pageIndex++) {
      ...
      for (let i = startPath; i < endPath; i++) {
        ...
        try {
          ...
          // Convert SVG to PNG with higher resolution for better quality
          const imageBuffer = await sharp(Buffer.from(svgString))
            .resize({
              width: imgSize * 8, // Increased multiplier for higher resolution to avoid pixelation
              height: imgSize * 8,
              fit: 'contain',
